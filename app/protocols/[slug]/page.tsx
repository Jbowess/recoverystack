...
export async function generateStaticParams() {
  ...
  return pages.filter((p) => p.template === 'protocols').map((p) => ({ slug: p.slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params;
  const page = await getPageByTemplateAndSlug('protocols', slug);
  if (!page) return { title: 'Not found' };
  return buildPageMetadata(page, '/protocols/' + slug);
}
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const schemaJsonLd = buildSchemaBundle(page, '/protocols/' + slug);
