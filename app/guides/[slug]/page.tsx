...
export async function generateStaticParams() {
  ...
  return pages.filter((p) => p.template === 'guides').map((p) => ({ slug: p.slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params;
  const page = await getPageByTemplateAndSlug('guides', slug);
  if (!page) return { title: 'Not found' };
  return buildPageMetadata(page, '/guides/' + slug);
}
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const schemaJsonLd = buildSchemaBundle(page, '/guides/' + slug);
