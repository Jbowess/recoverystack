...
export const revalidate = 900; // 15-min revalidation — news pages refresh more aggressively than evergreen

export async function generateStaticParams() {
  const pages = await getAllPublishedSlugs();
  return pages.filter((p) => p.template === 'news').map((p) => ({ slug: p.slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params;
  const page = await getPageByTemplateAndSlug('news', slug);
  if (!page) return { title: 'Not found' };
  return buildPageMetadata(page, '/news/' + slug);
}
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const schemaJsonLd = buildSchemaBundle(page, '/news/' + slug);
