...
export async function generateStaticParams() {
  ...
  return pages.filter((p) => p.template === 'metrics').map((p) => ({ slug: p.slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params;
  const page = await getPageByTemplateAndSlug('metrics', slug);
  if (!page) return { title: 'Not found' };
  return buildPageMetadata(page, '/metrics/' + slug);
}
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const schemaJsonLd = buildSchemaBundle(page, '/metrics/' + slug);
