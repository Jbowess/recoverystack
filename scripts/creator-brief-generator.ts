...
async function run() {
  ...
  for (const asset of (data ?? []) as any[]) {
    for (const creator of CREATOR_RELATIONSHIP_SEEDS.slice(0, 3)) {
      queued += 1;
      if (DRY_RUN) continue;
      ...
      const { error: upsertError } = await supabase.from('outreach_queue').upsert({
        ...
        body: [asset.summary ?? '', `Creator fit: ${creator.partnership_fit}`, `CTA: ${asset.cta_url ?? ''}`].join('\n\n'),
        cta_url: asset.cta_url ?? null,
        metadata: { creator_slug: creator.slug, audience_segment: creator.audience_segment, source_asset_type: 'creator_brief' },
