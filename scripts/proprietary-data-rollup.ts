...
async function run() {
  const [serpFeatures, competitorAnalyses, reviews, keywordRows] = await Promise.all([
    supabase.from('seo_serp_features').select('keyword,paa_questions,related_searches,featured_snippet_type,recommended_format').limit(200),
    supabase.from('seo_competitor_page_analyses').select('keyword,competitor_domain,required_entities,differentiating_entities').limit(200),
    supabase.from('seo_community_qa').select('source_platform,question,answer_snippet,sentiment,metadata').limit(200),
    supabase.from('seo_keyword_queue').select('primary_keyword,metadata').limit(200),
  ]);
  ...
  for (const row of (serpFeatures.data ?? []) as Array<any>) {
    ...
    serpWrites += 1;
    if (DRY_RUN) continue;
    const { error } = await supabase.from('seo_serp_snapshot_history').upsert({
      keyword: row.keyword,
      ...
      metadata: { source_table: 'seo_serp_features' },
    }, {
  ...
  for (const row of (competitorAnalyses.data ?? []) as Array<any>) {
    ...
    const { error } = await supabase.from('seo_competitor_page_snapshots').upsert({
      ...
      metadata: { source_table: 'seo_competitor_page_analyses' },
    }, {
  ...
  for (const row of (reviews.data ?? []) as Array<any>) {
    ...
    const { error } = await supabase.from('seo_community_topic_mentions').insert({
